"""Training v4 — extended FDS input vector (35 features) + anchor channels.

Same training recipe as train_v3 (same-cladding anchor-compatible mixup,
peak weight 1.0, relative-error loss, best-of-N greedy ensemble selection on
valid R²), with:
  - features_v4.build_params_v4: 16 baseline + 19 extended FDS/physics inputs
  - model_v4.KANAttentionLSTMv4: wider KAN parameter encoder, anchor as both
    static scalar input and per-timestep (delta, bump) channels
  - Saves to models_v4/

Usage: python train_v4.py
"""
...
from train_v3 import (
    ...
    LAMBDA_KAN_REG, ANCHOR_JITTER, MIXUP_ANCHOR_TOL,
    train_epoch, validate, pooled_r2,
)

N_CONTINUOUS = 2 + 13 + N_V4_FEATURES  # 34
MODEL_DIR = os.path.join(PROJECT_DIR, "models_v4")


def make_model(device):
    return KANAttentionLSTMv4(
...
def main():
    print("=" * 70)
    print("  BS8414 KAN-Attention-LSTM Surrogate — Training v4 (extended inputs)")
    print("=" * 70)
    ...
    print(f"\n  Input vector: {params_v4.shape[1]} features "
          f"(16 baseline + {N_V4_FEATURES} extended)")
